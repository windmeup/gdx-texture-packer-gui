...
//        sb.append("outputFormat=").append(settings.outputFormat).append("\n");
    ...
    for (Map.Entry<String, Polygon> entry : skeletonSettings.getAnimationBounds().entrySet()) {
      sb.append("animationBounds.").append(entry.getKey()).append('=');
      first = true;
      for (float v : entry.getValue().getVertices()) {
        if (first) {
          first = false;
        } else {
          sb.append(',');
...
//        settings.outputFormat = find(lines, "outputFormat=", defaultSettings.outputFormat);
    ...
    skeletonSettings.setDuration(find(lines, defaultSkeletonSettings.getDuration()));
    ...
    findPoints(lines, animationOffsets::put);
    skeletonSettings.setAnimationOffsets(animationOffsets);
    Map<String, Polygon> animationBounds = new TreeMap<>();
    findPolygons(lines, animationBounds::put);
    skeletonSettings.setAnimationBounds(animationBounds);
    ...
    // Legacy support and migrations section
    {
      ...
      if (version.isLower(new Version(4, 4, 0))) {
        if (inputDir != null && !inputDir.equals("")) {
          ...
          FileHandle[] subDirs = fileHandle.list(File::isDirectory);
  ...
  private static float find(Array<String> lines, float defaultValue) {
    String str = find(lines, "duration=", null);
    if (str != null) return Float.parseFloat(str);
    return defaultValue;
  }

  private static void findPoints(Array<String> lines, BiConsumer<String, Point> consumer) {
    String[] entry;
    String[] point;
    for (String line : lines) {
      if (line.startsWith("animationOffset.")) {
        entry = line.substring("animationOffset.".length()).split("=");
        point = entry[1].split(",");
  ...
  private static void findPolygons(Array<String> lines, BiConsumer<String, Polygon> consumer) {
    String[] entry;
    String[] values;
    float[] vertices;
    int index;
    for (String line : lines) {
      if (line.startsWith("animationBounds.")) {
        entry = line.substring("animationBounds.".length()).split("=");
        values = entry[1].split(",");
        vertices = new float[values.length];
        index = 0;
        for (String value : values) {
          vertices[index++] = Float.parseFloat(value);
        }
        consumer.accept(entry[0], new Polygon(vertices));
      }
    }
  }
}
